...
class Controller {
  constructor (model, db, optsGenerator, opts = {}) {
    const { defaultExcludes = [], relationshipCase = 'camelize'} = opts
    ...
    this.opts = {
      defaultExcludes, relationshipCase
    }
    new Array(
      'findAll',
      'handleQuery',
    ...
    try {
      ...
      Object.keys(obj).forEach(key => {
        if (
          ...
          obj[I[this.opts.relationshipCase](key + 'Id', false)] = obj[key].id
        }
      })
  ...
  findAll (req, res, next) {
    return this._model.findAndCountAll({
      attributes: this.generateAttributesClause(req),
      distinct: 'id',
    ...
    return Object.keys(this._model.associations)
      .map(modelName => {
        ...
        if (
          ...
          this.opts.defaultExcludes.indexOf(`${modelName}.*`) === -1
        ) {
  ...
  generateLimitStatement (req) {
    const page = req.query.page || {}
    return page.limit === '!' ? null : +page.limit || 20
  }
  ...
  async handleQuery (req, res, next) {
    ...
    try {
      await this.onBeforeGetQuery(req)
      ...
      await this.onAfterGetQuery(req, rows)
      rows.pagination = this.generatePaginationObject(req, count)
      if(req.query.skipJsonApi) {
  ...
  async handleQueryById (req, res, next) {
    ...
    try {
      await this.onBeforeGetById(req)
      let data = await this.findById(...arguments)
      if (!data) {
        return this.handleError(
          {
            code: 404,
      ...
      await this.onAfterGetById(req, data)
      if(req.query.skipJsonApi) {
  ...
  async handleCreate (req, res, next) {
    try {
      ...
      await this.onBeforeCreate(req, modelObj)
      ...
      await this.onAfterCreate(req, result)
      if(req.query.skipJsonApi) {
  ...
  async handleUpdateById (req, res, next) {
    try {
      ...
      await this.onBeforeUpdate(req, modelObj)
      ...
      await this.onAfterUpdate(req, result)
      if(req.query.skipJsonApi) {
        return res.json(result)
      }
      res.json(this.serialize(result))
  ...
  // all these hooks can return a promise
  onBeforeGetQuery(req) {}
  onAfterGetQuery(req, model) {}

  onBeforeGetById(req) {}
  onAfterGetById(req, model) {}
 
  onBeforeCreate(req, model) {}
  onAfterCreate(req, model) {}

  onBeforeUpdate(req, model) {}
  onAfterUpdate(req, model) {}
}
